...
function CartBadge() {
  return (
    <span className="relative inline-flex items-center">
      ...
      <span className="pointer-events-none absolute -top-2 -right-2 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-[#ED0B6D] px-1 text-[10px] font-bold text-white" aria-label="1 items in cart">1</span>
    </span>
  );
}
      ...
      <div className="mx-auto max-w-screen-2xl lg:px-3">
        <div className="relative flex h-16 items-center justify-between lg:h-24">
          ...
          <div className="flex w-full items-center justify-between px-2 lg:hidden">
            <button type="button" onClick={() => setMobileOpen((v) => !v)} className="inline-flex h-16 items-center p-2 text-white focus:outline-none" aria-label="Menu">
              <svg className="h-7 w-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" d="M4 7h16M4 12h16M4 17h16" /></svg>
            </button>
            <a href={home} className="flex items-center">
              <Logo className="h-[40px] w-[40px]" />
              <span className="ml-2 text-[13px] font-semibold uppercase text-white">{nav.brand}</span>
            </a>
            <div className="flex items-center gap-1 text-white">
              <QuickIcon label={nav.quickIcons.zakat} icon="/assets/icon-zakat.png" />
              <span className="px-1"><CartBadge /></span>
              <button type="button" onClick={donate} className="btn-matw h-10">{nav.donate}</button>
            </div>
          </div>
          ...
          <div className="hidden w-full items-center justify-between lg:flex">
            <a href={home} className="flex items-center pl-1">
              ...
              <span className="ml-2 text-[14px] font-semibold uppercase text-white">{nav.brand}</span>
            </a>
            <ul className="flex h-24 items-center">
              <li className="flex h-24 items-center px-2"><LanguageSwitcher nav={nav} /></li>
              <QuickIcon label={nav.quickIcons.zakat} icon="/assets/icon-zakat.png" big />
              <QuickIcon label={nav.quickIcons.meals} icon="/assets/icon-meal.png" big />
              ...
              {nav.menus.map((menu) => (
                <li key={menu.label} className="group relative flex h-24 items-center border-l border-white/10 px-3">
                  <span className="flex cursor-pointer items-center gap-1 text-[14px] uppercase text-white transition-all duration-200 group-hover:text-white/80">
                    {menu.label}<Chevron className="mt-[2px]" />
                  </span>
                  <ul className="invisible absolute left-0 top-24 z-[1000] flex min-w-[270px] flex-col rounded-md bg-white p-4 opacity-0 shadow-md transition-all duration-150 group-hover:visible group-hover:opacity-100">
                    {menu.items.map((item) => (
                      <li key={item.label} className="group/sub relative flex items-center justify-between hover:bg-stone-100">
                        <a href={item.href} className="block px-3 py-1 text-[15px] text-[#777] transition-all hover:text-[#00a3da]">{item.label}</a>
                          ...
                          <ul className="invisible absolute left-full top-0 -ml-2 flex min-w-[260px] flex-col rounded-md bg-white p-4 opacity-0 shadow-md transition-all group-hover/sub:visible group-hover/sub:opacity-100">
                            {item.children.map((c) => (
                              <li key={c.label} className="hover:bg-stone-100">
                                <a href={c.href} className="block px-3 py-1 text-[15px] text-[#777] transition-all hover:text-[#00a3da]">{c.label}</a>
                              </li>
                            ))}
                          </ul>
                        )}
              ...
              <li className="flex h-24 items-center border-l border-white/10 px-4 text-white"><CartBadge /></li>
              <li className="flex items-center pl-3">
                <div className="flex flex-col items-center">
                  <button type="button" onClick={donate} className="btn-matw h-11 px-6 text-[14px]">{nav.donate}</button>
                  <a href={`tel:${nav.phone.replace(/[^+\d]/g, "")}`} className="mt-1 text-[12px] text-white/90">{nav.phone}</a>
                </div>
              </li>
            </ul>
          </div>
          ...
          {nav.menus.map((menu) => (
            <li key={menu.label} className="border-b border-[#81c3ed] px-4 py-2">
              ...
              <ul className="mt-1 rounded-md bg-[#65bff7] px-3 py-1">
                {menu.items.map((item) => (
                  <li key={item.label}><a href={item.href} className="block py-1 pl-2 text-[15px] text-white hover:text-white/80">{item.label}</a></li>
                ))}
              </ul>
            </li>
          ))}
          <li className="flex flex-col items-center py-3">
            <a href={`tel:${nav.phone.replace(/[^+\d]/g, "")}`} className="py-1 text-xl text-white">{nav.phone}</a>
            <span className="px-3 text-white/90">{nav.hours}</span>
          </li>
        </ul>
      )}
    </nav>
...
function QuickIcon({ label, icon, big = false }) {
  ...
  return (
    <li className={`flex ${big ? "h-24 flex-col" : "flex-col"} items-center justify-center border-l border-white/10 px-2 text-white`}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={icon} alt={label} className={`${size} rounded-full object-cover`} />
      <span className={`mt-1 max-w-[80px] text-center leading-tight ${big ? "text-[12px]" : "text-[10px]"}`}>{label}</span>
    </li>
  );
}

function LanguageSwitcher({ nav }) {
  return (
    <div className="group relative">
      ...
      <ul className="invisible absolute -right-3 top-6 z-[99] w-[150px] flex-col rounded-md bg-white p-3 opacity-0 shadow-md transition-all group-hover:visible group-hover:opacity-100">
        {nav.languages.map((l) => (
          <li key={l.label}>
            <a href={l.href} className="block py-1 text-[15px] text-[#093484] hover:text-[#00a3da]">{l.label}</a>
          </li>
        ))}
      </ul>
    </div>
