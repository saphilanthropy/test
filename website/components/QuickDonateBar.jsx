...
export default function QuickDonateBar({ content }) {
  const t = content.quickDonate;
  ...
  const handleDonate = async () => {
    if (frequency === "monthly" && !amount) {
      alert(t.monthly + ": " + t.amountPlaceholder);
      return;
    }
    setLoading(true);
    ...
    <section className="matw-gradient w-full px-3 py-3 md:px-6 md:py-6">
      <div className="mx-auto w-full md:container">
        ...
        <div className="mb-2 flex justify-center gap-2 lg:justify-start lg:px-2">
          {[["one_time", t.oneTime], ["monthly", t.monthly]].map(([val, label]) => (
            <button key={val} type="button" onClick={() => setFrequency(val)}
              className={`rounded-full px-4 py-1 text-sm font-semibold transition-colors ${frequency === val ? "bg-white text-[#093484]" : "bg-white/20 text-white hover:bg-white/30"}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-12 gap-2 lg:grid-cols-[1.5fr_1fr_1.5fr_1fr_1fr] lg:px-2">
          <div className="order-3 col-span-12 flex h-[42px] items-center rounded-md bg-white md:col-span-5 md:order-1 lg:col-span-1">
            <span className="flex items-center gap-1 border-r px-3 text-[15px] font-medium text-[#777]">🇺🇸 USD</span>
            <input type="number" value={amount} onChange={(e) => { setAmount(e.target.value); setActive(null); }} placeholder={t.amountPlaceholder} className="h-[42px] w-full truncate p-2 pl-2 text-[#777] focus:outline-none" />
          </div>
          ...
          <div className="order-2 col-span-12 grid grid-cols-4 gap-2 md:order-3 lg:order-2 lg:col-span-1 lg:flex">
            {presetAmounts.map((amt, i) => (
              <button key={amt} onClick={() => { setActive(i); setAmount(amt.replace("$", "")); }}
                className={`flex min-w-[60px] items-center justify-center rounded-md px-2 py-2 text-sm transition-all duration-200 md:px-4 md:py-3 ${active === i ? "bg-[#f60362] text-white" : "bg-white text-[#093484] hover:bg-[#e2eff3]"}`}>
                {amt}
              </button>
            ))}
          </div>
          ...
          <div className="order-1 col-span-12 md:order-2 md:col-span-7 lg:order-3 lg:col-span-1">
            <select value={program} onChange={(e) => setProgram(e.target.value)} className="h-[42px] w-full rounded-md px-3 text-[#777] focus:outline-none">
          ...
          <div className="order-5 col-span-6 lg:col-span-1">
            <button type="button" onClick={handleDonate} disabled={loading} className={`btn-matw h-[42px] w-full ${loading ? "opacity-70" : ""}`}>
